...
const Signup: React.FC<SignupProps> = ({ onSwitch, onNavigateToLegal }) => {
    ...
    return (
        <div>
            <h2 className="text-2xl font-bold text-center text-slate-800 dark:text-slate-100 mb-6">Create an Account</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-start pt-2">
                    <div className="flex items-center h-5">
                        <input id="terms" name="terms" type="checkbox" checked={termsAccepted} onChange={(e) => setTermsAccepted(e.target.checked)} className="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-slate-300 dark:border-slate-500 rounded bg-slate-100 dark:bg-slate-700" />
                    </div>
                    <div className="ml-3 text-sm">
                        <label htmlFor="terms" className="text-slate-500 dark:text-slate-400">
                            I agree to the{' '}
                            <button type="button" onClick={() => onNavigateToLegal('terms')} className="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300">Terms of Service</button> and{' '}
                            <button type="button" onClick={() => onNavigateToLegal('privacy')} className="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300">Privacy Policy</button>.
                        </label>
                    </div>
                </div>
                 <button
