...
const Toast: React.FC<ToastProps> = ({ message, type, onClose }) => {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setProgress(prev => Math.max(prev - (100 / (5000 / 100)), 0));
        }, 100);
            ...
            <div className="p-4">
                <div className="flex items-start">
                    ...
                    <div className="ml-4 flex-shrink-0 flex">
                        <button onClick={onClose} className="rounded-md inline-flex text-slate-400 hover:text-slate-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
             ...
             <div className={`h-1 ${BG_COLORS[type]}`} style={{ width: `${progress}%` }}></div>
        </div>
    );
};
